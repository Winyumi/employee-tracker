...
let query = {
    ...
    viewDepartmentsTotalBudget: () =>
        db(`SELECT department.id, department.name, CONCAT("$", SUM(role.salary)) AS budget FROM ((employee LEFT JOIN role ON employee.role_id = role.id) LEFT JOIN department ON role.department_id = department.id) GROUP BY department.id`),
